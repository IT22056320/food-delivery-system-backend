...
exports.getOrderById = async (req, res) => {
  try {
    ...
    console.log(
      `Found order: ${order._id}, restaurant ID: ${
        order.restaurantId || order.restaurant_id
      }`
    );

    // Use either restaurantId or restaurant_id field depending on which one exists
    const restaurantId = order.restaurantId || order.restaurant_id;

    if (!restaurantId) {
      console.log("Order has no restaurant ID");
      // Return the order even if restaurant is not found
      return res.status(200).json({
        ...order.toObject(),
        restaurant: { name: "Unknown Restaurant" },
        _warning: "Restaurant information not available",
      });
    }
    ...
    try {
      ...
      if (!restaurant) {
        ...
        return res.status(200).json({
          ...order.toObject(),
          restaurant: { name: "Unknown Restaurant" },
          _warning: "Restaurant not found",
        });
      }

      // Only check authorization if we found the restaurant
      if (
      ...
      // Return order with restaurant info
      return res.status(200).json({
        ...order.toObject(),
        restaurant: {
          id: restaurant._id,
      ...
      // Return the order even if there was an error fetching the restaurant
      return res.status(200).json({
        ...order.toObject(),
        restaurant: { name: "Unknown Restaurant" },
        _warning: "Error fetching restaurant information",
      });
    }
  } catch (error) {
...
exports.getPendingOrders = async (req, res) => {
  try {
    ...
    if (restaurant.ownerId !== req.user._id && req.user.role !== "admin") {
      res
        .status(403)
        .json({ error: "Not authorized to view orders for this restaurant" });
      return;
    }
...
exports.getPopularMenuItems = async (req, res) => {
  try {
    ...
    if (restaurant.ownerId !== req.user._id && req.user.role !== "admin") {
      res
        .status(403)
        .json({ error: "Not authorized to view stats for this restaurant" });
      return;
    }
